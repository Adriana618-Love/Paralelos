 ...
 * Ejecutar:
 *    mpiexec -n <p> ./mpi_odd_even g <global_n> 
...
int main(int argc, char* argv[]) {
   ...
   local_A = (int*) malloc(local_n*sizeof(int)); /* INicializo el vector a ordenar */
   if (g_i == 'g') { /* Si la lista será generada */
      Generate_list(local_A, local_n, my_rank);
      //Print_local_lists(local_A, local_n, my_rank, p, comm);
   } else { /* Si la lista será ingresada */
...
void Get_args(int argc, char* argv[], int* global_n_p, int* local_n_p, 
         ...
         *gi_p = argv[1][0];
         if (*gi_p != 'g' && *gi_p != 'i') {
            ...
            *global_n_p = atoi(argv[2]);
            if (*global_n_p % p != 0) { /* Check if n is divisible for p*/
               Usage(argv[0]);
               *global_n_p = -1;
            }
         }
   ...
   MPI_Bcast(gi_p, 1, MPI_CHAR, 0, comm); /* Comunico a todos que la lista será generada o recibida */
   MPI_Bcast(global_n_p, 1, MPI_INT, 0, comm); /* Comunico a todos cual es el n global */
   ...
   *local_n_p = *global_n_p/p; /*Divido el programa*/
#  ifdef DEBUG
